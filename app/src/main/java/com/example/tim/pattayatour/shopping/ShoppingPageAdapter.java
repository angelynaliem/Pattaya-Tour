...
public class ShoppingPageAdapter extends FragmentPagerAdapter {

    // Context of the app
    private Context mContext;

    /**
     * Create a new {@link ShoppingPageAdapter} object
     *
     * @param context is the content of the app
     * @param fm      is the fragment manager that will keep each fragment's state in the adapter
     *                across swipes
     */
    public ShoppingPageAdapter(Context context, FragmentManager fm) {
        super(fm);
        mContext = context;
    }
    ...
    public Fragment getItem(int position) {
        if (position == 0) {
            return new ShoppingInformationFragment();
        } else if (position == 1) {
